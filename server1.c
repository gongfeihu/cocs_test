...
void handle_client(int client_socket) {
    printf("Handling client connection on server1\n");
    ...
    // 收集当前目录中的所有FastFp
    int fastfp_count;
    uint64_t *all_fastfps = get_all_fastfps_from_dir("./server1", &fastfp_count);
    ...
    for (int i = 0; i < match_count; i++) {
        char chunk_path[512];
        snprintf(chunk_path, sizeof(chunk_path), "./server1/%016lx.chunk", matching_fastfps[i]);
    ...
    for (int i = 0; i < upload_count; i++) {
        ...
        snprintf(chunk_filename, sizeof(chunk_filename), "./server1/%016lx.chunk", fastfp);
    ...
    printf("Finished handling client on server1\n");
}

// fastcdc.c - FastCDC算法实现
#include <stdint.h>
#include <string.h>
#include <openssl/md5.h>
#include "fastcdc.h"

// 预定义 Gear 表（与原 fastcdc.h 中一致）
uint64_t GEARv2[256] = {
    ...
    0x5cb1a8a10079cb23, 0xc52b57c63ff9a0ca, 0x0f35659fc64e0143,
    0xbdf325035c594f38, 0xd1f8101a35320afd, 0xe899458626d703f8,
    0xbf97f530ac049837, 0x0895021a6620ae70, 0x948596cd24280401,
    ...
    0x501c523d559d7d12, 0x9b7e72aa3d7f1a65, 0x025554c1d398077a,
    0xcdd8af6cd9c471e1, 0x039bea30bd9a49e,  0x233b737e0eeee721,
    0xd6d57c6896b14ed4, 0x8dabe80e8c70e265, 0x1b859ed8c291ddbc,
    0x6b3385f41e0a598b, 0xcf05250d14390e94, 0xc58d5e7d9c8b9f73,
    0x49a5c7a9ba27febc, 0x4841a129f0804005, 0x0e7ffa313ce3144a,
    ...
    0x09a398f26b33f24a, 0x74595977a450ca7b, 0x4e8468ce85680390,
    ...
    0x0b3646a9d0111233, 0xf0af68353b17c0c8, 0xc920f56447f90673,
    0x8960c8168641fe16, 0x0fee91d454a19219, 0xb25b446e135cf6b2,
    ...
    0x7801a00c795d859d, 0x041bdd48db6ae14a, 0x5fa8107e14c841fb,
    0xd0e4bdea28bef85c, 0x77bdb5eb30614b89, 0xdefa9fd302bbd858,
    0x08a54bfa54688dad, 0x682ec11a915f0980, 0xc4af0b1c0ffec719,
    0xf76fd41604e89104, 0xeb01bf3d9ced6817, 0xa87180c091474c6e,
    0x351bdb3557277969, 0xee81b4ce09b723aa, 0x03bd353a36b05adb,
    ...
    0x2490253782ef8a57, 0x0ad4d59ff8fa5f4c, 0x1c01a62b4c726533,
    ...
    0xaa38a80cc6c609f3, 0x5f97356ca50a5986, 0x0220a1b6bc88caeb,
    ...
    0x9d36a507aa427257, 0x069b7818b57dbbf8, 0x21102213d2cae2cd,
    ...
    0x02443385d4c4e195, 0x2ee724f1ad94f68e, 0x0a4491e380c5cf7b,
    0xeb48a728aaf18d2e,
};

uint64_t LEARv2[256];

uint64_t FING_GEAR_08KB_ls_64 = 0x0000d93003530000ULL << 1;
uint64_t FING_GEAR_02KB_ls_64 = 0x0000d90003530000ULL << 1;
uint64_t FING_GEAR_32KB_ls_64 = 0x0000d9f003530000ULL << 1;
uint64_t FING_GEAR_08KB_64 = 0x0000d93003530000ULL;
uint64_t FING_GEAR_02KB_64 = 0x0000d90003530000ULL;
uint64_t FING_GEAR_32KB_64 = 0x0000d9f003530000ULL;
...
uint32_t Mask_15 = 0;
uint32_t Mask_11 = 0;
uint64_t Mask_11_64 = 0, Mask_15_64 = 0;

uint32_t MinSize = 0;
uint32_t MinSize_divide_by_2 = 0;
uint32_t MaxSize = 0;
int sameCount = 0;
int tmpCount = 0;
int smalChkCnt = 0;  // 记录小于8KB的分块

int (*chunking)(unsigned char *p, int n, uint64_t *feature, uint64_t *weakhash) = NULL;
...
int normalized_chunking_64(unsigned char *p, int n, uint64_t *feature, uint64_t *weakhash) {
    ...
    if (n <= (int)MinSize) {
        for (int j = 0; j < n; j++) {
            fingerprint = (fingerprint << 1) + (GEARv2[(unsigned char)p[j]]);
        }
    ...
    while (i < Mid) {
        fingerprint = (fingerprint << 1) + (GEARv2[(unsigned char)p[i]]);
